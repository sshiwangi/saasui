...
import ThemeCustomizer from "./DashboardThemePicker";
...
export default function ThemeCustomizerExample() {
  ...
  useEffect(() => {
    fetch(
      `/api/source?path=${encodeURIComponent(
        "src/app/allcomponents/dashboardthemepicker/DashboardThemePicker.tsx"
      )}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="space-y-8 h-full">
      <CodePreview component={ThemeCustomizer} code={source} />
    </div>
  );
}
